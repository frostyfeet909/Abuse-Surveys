...
VERBOSE = 1  # Level of verbosity (format: 0-2)
lock_file = Lock()
...
class Survey_Destroyer(Thread):
    ...
    def __init__(self, survey, ID, repeats, protect=False):
        """
...
class Monkey_Destroyer(Survey_Destroyer):
    ...
    def __init__(self, survey, ID, repeats, protect=False):
        """
            ...
            protect - DOes survey have protection enabled : Boolean
        """
        super().__init__(survey, ID, repeats, protect)
        self.lock = Time_Lock()
